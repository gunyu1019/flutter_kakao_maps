...
extension RouteControllerHandler {
    func routeHandle(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        func requireLayer() -> RouteLayer? {
            guard let layer else {
                result(missingNativeResource(method: call.method, resource: "route layer", id: layerId))
                return nil
            }
            return layer
        }

        func requireRoute() -> Route? {
            guard requireLayer() != nil else { return nil }
            guard let route else {
                result(missingNativeResource(method: call.method, resource: "route", id: routeId))
                return nil
            }
            return route
        }
        ...
        case "removeRouteLayer":
            ...
            removeRouteLayer(layerId: layerId, onSuccess: result)
        ...
        case "addRoute":
            ...
            addRoute(layer: layer, route: asRouteOption(payload: asDict(arguments!["route"]!)), onSuccess: result)
        case "addMultipleRoute":
            guard let layer = requireLayer() else { return }
            addRoute(layer: layer, route: asRouteMultipleOption(payload: asDict(arguments!["route"]!)), onSuccess: result)
        case "removeRoute":
            guard let layer = requireLayer(), requireRoute() != nil, let routeId else { return }
            removeRoute(layer: layer, routeId: routeId, onSuccess: result)
        case "changeRoute":
            guard let route = requireRoute() else { return }
        ...
        case "changeVisibleAllRoute":
            guard let layer = requireLayer() else { return }
            changeRouteLayerVisible(layer: layer, visible: asBool(arguments!["visible"]!), onSuccess: result)
